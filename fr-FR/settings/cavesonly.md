# 🚫 Désactiver les mobs hostiles

{% hint style="info" %}
<p align="center">Par défaut, Cobblemon Realms limite l'apparition des mobs hostiles principalement aux grottes grâce au mod <strong>In Control!</strong>. Si vous souhaitez retrouver un comportement plus proche de Minecraft vanilla, il suffit de retirer ce mod.</p>
{% endhint %}

***

## ⚙️ Étapes à suivre

1. Ouvrez le dossier `mods` de votre installation Cobblemon Realms.
2. Recherchez le fichier `InControl-*.jar`.
3. Supprimez-le.
4. Redémarrez Minecraft.

{% hint style="success" %}
Les mobs hostiles réapparaîtront alors normalement à la surface, comme dans Minecraft vanilla.
{% endhint %}

***

## 📌 Conséquences

|         Changement        |                       Impact                       |
| :-----------------------: | :------------------------------------------------: |
| 🌙 Apparitions en surface | Les monstres peuvent de nouveau apparaître la nuit |
|       ⚔️ Difficulté       |        Les nuits deviennent plus dangereuses       |
|      🏞️ Exploration      |    Le gameplay se rapproche davantage du vanilla   |
|   👹 Population hostile   |     Davantage de mobs apparaissent globalement     |

***

## ⚠️ Important

{% hint style="warning" %}
#### 🌐 Sur un serveur

Cette modification doit être effectuée côté serveur. Supprimer le mod uniquement côté client n'aura aucun effet sur les apparitions de mobs.
{% endhint %}

{% hint style="danger" %}
### ⚖️ Équilibrage du modpack

Cobblemon Realms a été conçu autour de cette limitation. Retirer **In Control!** peut augmenter sensiblement la difficulté et modifier l'expérience d'exploration.
{% endhint %}

***

{% hint style="success" %}
### Nous contacter

<p align="center">Si vous avez des questions, des suggestions ou des modifications à proposer, n'hésitez pas à nous rejoindre sur <a href="https://discord.gg/kb8NSTF45n">Discord</a> et à contacter directement <strong>@FabLeKebab</strong> sur le serveur pour tout ce qui concerne le wiki, ou <strong>@Levels</strong> pour tout ce qui concerne le modpack.</p>
{% endhint %}
